...
export function Editor(props: any) {
    ...
    function onKeyUp(e: any) {
        const id = window.getSelection()?.anchorNode?.parentElement?.parentElement?.id;
        for (let i = 0; i < editorStore.components.length; i++) {
            if (editorStore.components[i].id === id) {
                const newValue = window.getSelection()?.anchorNode?.parentElement?.innerText;
    ...
    const load = () => {
        let loadedComponents = JSON.parse(localStorage.getItem("components") as string);

        loadedComponents.map((component: any) => {
            const componentType = componentStore.filter((registeredComponent) => registeredComponent.name === component.type);
    ...
    const reorder = useCallback((dragIndex: number, hoverIndex: number) => {
        setComponents((prevCards: Component[]) => {
            ...
            newDocument[dragIndex] = [newDocument[hoverIndex], (newDocument[hoverIndex] = newDocument[dragIndex])][0];
            ...
            <div contentEditable="true" onKeyUp={onKeyUp}>
                <DndProvider backend={HTML5Backend}>
                    {components.map((component, index) => (
                        <div>
                            <Wrapper key={component.id} id={component.id} index={index} reorder={reorder}>
                                {React.createElement(
